...
public class NavigationController {
	...
	public static void obsOfficeVisit() throws IOException {
		ExternalContext ctx = FacesContext.getCurrentInstance().getExternalContext();
		if (ctx != null) {
			ctx.redirect("/iTrust/auth/hcp-uap/viewObstetricsVisit.xhtml");
		}
	}

	//TO DO: create redirect page
	public static void obstetricsOfficeVisit() throws IOException {
		ExternalContext ctx = FacesContext.getCurrentInstance().getExternalContext();
		if (ctx != null) {
			ctx.redirect("/iTrust/auth/hcp-uap/viewObstetricOfficeVisits.xhtml");
		}
	}

	public static void ultrasoundEntry() throws IOException {
		ExternalContext ctx = FacesContext.getCurrentInstance().getExternalContext();
		if (ctx != null) {
			ctx.redirect("/iTrust/auth/hcp-uap/ultrasounds/viewUltrasounds.xhtml");
		}
	}
	...
	public static void childbirthVisit() throws IOException {
		ExternalContext ctx = FacesContext.getCurrentInstance().getExternalContext();
		if (ctx != null) {
			ctx.redirect("/iTrust/auth/hcp-uap/viewOnlyObstetricsRecords.xhtml");
		}
	}

	public static void childbirth() throws IOException {
		ExternalContext ctx = FacesContext.getCurrentInstance().getExternalContext();
		if (ctx != null) {
			ctx.redirect("/iTrust/auth/hcp-uap/viewOnlyObstetricsRecords.xhtml");
		}
