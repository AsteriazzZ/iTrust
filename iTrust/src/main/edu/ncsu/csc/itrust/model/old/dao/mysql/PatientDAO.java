...
public class PatientDAO {
	...
	public void editObstetricEligibility(boolean eligibility, long patientMID) throws DBException {
		try (Connection conn = factory.getConnection();
			 PreparedStatement ps = conn.prepareStatement("UPDATE patients SET ObstetricEligibility=? WHERE MID = ?;")) {
			ps.setBoolean(1, eligibility);
			ps.setLong(2, patientMID);
			ps.executeUpdate();
		} catch (SQLException e) {
			throw new DBException(e);
	...
	public boolean getPatientObstetricEligibility(long patientMID) throws DBException{
		try (Connection conn = factory.getConnection();
			 PreparedStatement ps = conn.prepareStatement("SELECT ObstetricEligibility FROM " + "patients WHERE MID = ?;")) {
			ps.setLong(1, patientMID);
			ResultSet results = ps.executeQuery();
			boolean eligibility = results.next() ? results.getBoolean(1): null;
			results.close();
			return eligibility;
		} catch (SQLException e) {
			throw new DBException(e);
