...
public class PatientMySQLConverter implements DataBean<Patient>, Serializable{
	...
	public Long getByName(String firstName, String lastName) throws DBException {
		Patient ret = null;
		Connection conn = null;
		PreparedStatement pstring = null;
		ResultSet results = null;
		String stmt = "";
		stmt = "SELECT patients.MID AS MID, patients.firstName AS firstName, patients.lastName AS lastName FROM patients WHERE patients.firstName=? AND patients.lastName=?;";
		try {
			List<Patient> list = null;
			conn=ds.getConnection();
			pstring = conn.prepareStatement(stmt);
			pstring.setString(1, firstName);
			pstring.setString(2, lastName);
			results = pstring.executeQuery();
			list = ((PatientSQLConvLoader)loader).loadList2(results);
			if(!list.isEmpty()){
				ret = list.get(0);
			}
			return ret.getMID();
