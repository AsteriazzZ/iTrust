...
public class ChildbirthMySQLTest extends TestCase {
    private DataSource ds;
    private ChildbirthMySQL ovsql;
    private ChildbirthMySQL ovsql2;
    ...
    @Test
    public void testGetChildForVisit() throws Exception {
        List<Childbirth> listNull = ovsql.getChildbirthsForVisit(-1L);
        assertEquals(listNull, null);
        List<Childbirth> list101 = ovsql.getChildbirthsForVisit(101L);
        assertEquals(list101.size(), 0);
    }
    ...
    public void testGetAll() throws Exception {
        ...
        Childbirth cv2 = new Childbirth();
        cv2.setVisitID(2L);
        cv2.setDate(LocalDateTime.now());
        cv2.setSex("Female");
        cv2.setName("Oprah Winfrey");
        cv2.setDeliveryType("Vaginal Delivery");
        assertTrue(ovsql.add(cv2));

        List<Childbirth> list = ovsql.getAll();
